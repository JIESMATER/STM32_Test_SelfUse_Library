...
//        usb_printf("sgp30 start fail\r\n");
    ...
    if (status != HAL_OK) {
//       usb_printf("I2C Master Receive fail\r\n");
        return -1;
    }
    
    /* 校验接收的测量数据 */
    if (CheckCrc8(&recv_buffer[0], 0xFF) != recv_buffer[2]) {
//        usb_printf("co2 recv data crc check fail\r\n");
        return -1;
    }
    if (CheckCrc8(&recv_buffer[3], 0xFF) != recv_buffer[5]) {
//       usb_printf("tvoc recv data crc check fail\r\n");
        return -1;
    }
 ...
 * @brief	读取一次二氧化碳浓度
 * @param	none
 * @retval	成功返回二氧化碳数值，失败返回0
*/
uint16_t sgp30_Get_CO2Value(void)
{
	 
if( -1 == sgp30_read()) 
	{
//	usb_printf("sgp30 read fail\r\n");
	return 0;
		 }
else{
	HAL_Delay(100);
