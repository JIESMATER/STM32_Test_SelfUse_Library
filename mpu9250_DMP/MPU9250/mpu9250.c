...
//			
				sprintf(str_buff,"%.2f    ",roll);
				OLED_ShowString(40,2,(uint8_t *)str_buff,16);//在第二行显示横滚角
				sprintf(str_buff,"%.2f    ",pitch);
				OLED_ShowString(48,4,(uint8_t *)str_buff,16);//在第三行显示俯仰角
					sprintf(str_buff,"%.2f    ",yaw);
				OLED_ShowString(32,6,(uint8_t *)str_buff,16);//在第四行显示航向角
        }
    }
 }
